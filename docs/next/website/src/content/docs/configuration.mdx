...
Remote attach uses Eternal Terminal for its data connection by default, and manages its SSH bootstrap connection with a temporary keepalive and connection-reuse fallback.
...
`transport` selects how the persistent `herdr --remote` data connection is carried:

- `"et"` (default) uses [Eternal Terminal](https://eternalterminal.dev/). `et` must be installed on both the local machine and the remote host. Herdr starts one persistent, auto-reconnecting `et` session that forwards a loopback TCP port to a small remote bridge over the server's client socket, so the connection survives network drops and suspends. The remote TCP port is derived deterministically from the session name.
- `"ssh"` uses a plain `ssh` stdio bridge (one `ssh` process per client connection), the original behavior.

`et_corp_internal` (default `false`) switches the `et` transport to a corporate/VPNless flavour: herdr passes `--ssh-socket`, `--forward-ssh-agent`, and an `x2ssh` `ProxyCommand` for VPNless auth, and reaches the et server on the corporate port. The x2p broker socket is taken from the `X2P_SOCK` environment variable when set. Leave it `false` for bare `et`.

`binary_path` points at a local `herdr` binary (built for the remote's OS/arch) to install when a remote host has no matching `herdr`. When unset, herdr copies the local binary if the remote matches this machine's platform, otherwise downloads the released binary. The `HERDR_REMOTE_BINARY` environment variable overrides it.

Either transport still bootstraps over `ssh` (detecting the platform and installing/matching the remote `herdr` binary); that bootstrap now runs in a single `ssh` round trip for an already-provisioned remote, so it authenticates once instead of once per probe.

When `manage_ssh_config` is enabled, `herdr --remote` writes a private temporary SSH config that includes your `~/.ssh/config` and `/etc/ssh/ssh_config` first, then adds fallback `ServerAliveInterval` and `ServerAliveCountMax` values. Your own keepalive settings win. Herdr also uses a private per-attach OpenSSH control socket to reuse the first authenticated connection. Set `manage_ssh_config = false` to run remote attach through plain `ssh` without Herdr's generated config or control socket.
...
`confirm_close` controls whether closing a workspace asks for confirmation. `prompt_new_tab_name` controls whether new tabs ask for a label first; it defaults to `false`, so new tabs are created immediately with generated names. Set it to `true` to open the rename dialog first.
