...
| Move between panes | `ctrl+h/j/k/l` (or `prefix+h/j/k/l`) |
...
| Resize the focused pane | `ctrl+shift+h/j/k/l` |
...
Herdr ships direct pane movement and resize on `ctrl+h/j/k/l` and `ctrl+shift+h/j/k/l`, keeping the `prefix+…` bindings as fallbacks. These `ctrl` chords are ergonomic but overlap with control codes some programs use inside a pane (`ctrl+h` is backspace, `ctrl+j` is newline/enter, `ctrl+l` clears the screen), and Herdr captures them before the pane sees them. If that bothers a program you use, drop the `ctrl+…` entries and rely on the prefix bindings, or move to the collision-free `ctrl+alt` family:
