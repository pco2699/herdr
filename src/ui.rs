...
mod tests {
    ...
    fn keybind_help_shows_unset_for_optional_actions() {
        ...
        assert!(panes
            .iter()
            .any(|(key, label)| key == "ctrl+h / prefix+h" && label.as_ref() == "focus pane left"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "ctrl+j / prefix+j" && label.as_ref() == "focus pane down"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "ctrl+k / prefix+k" && label.as_ref() == "focus pane up"));
        assert!(
            panes
                .iter()
                .any(|(key, label)| key == "ctrl+l / prefix+l"
